...
import { X, Clock, Wifi } from 'lucide-react-native';
import { attendanceService, TodayShift, TodayAttendance } from '../../services/attendanceService';
...
export default function CheckOutScreen() {
    ...
    const handleCheckOut = async () => {
        ...
        // Kiểm tra WiFi trước khi check-out
        if (!wifiInfo) {
            Alert.alert(
                'Cần kết nối WiFi',
        ...
        try {
            ...
            await attendanceService.checkOut(photo.uri, wifiInfo || undefined);

            Alert.alert(
                'Thành công',
                `Check-out thành công lúc ${new Date().toLocaleTimeString('vi-VN')}\nThời gian làm việc: ${calculateWorkingHours()}`,
                [
                    {
                        text: 'OK',
                        onPress: () => router.back(),
    ...
    const requestCameraPermission = async () => {
        if (!permission?.granted) {
            ...
            if (!result.granted) {
                Alert.alert('Cần quyền truy cập', 'Vui lòng cấp quyền camera để check-out');
                return false;
            }
        }
        return true;
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.bottomSheet}>
                <Text style={styles.title}>Check-out ca làm việc</Text>
                    ...
                    <>
                        ...
                        {attendance?.checkInTime && (
                            <View style={styles.timeCard}>
                                ...
                                <View style={styles.timeRow}>
                                    <Text style={styles.timeLabel}>Thời gian làm việc:</Text>
                ...
                <View style={[
                    ...
                    <View style={styles.wifiHeader}>
                        <Wifi size={18} color={wifiInfo ? '#f59e0b' : '#ef4444'} />
                        <Text style={[
